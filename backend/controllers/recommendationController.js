  ...
  try {
    ...
    if (preferences.favoriteActor) {
      try {
        ...
        console.warn("⚠️ Failed to fetch actor ID:", err.message);
      }
    }
    ...
    for (const movieId of ratedOrSavedMovieIds) {
      try {
        ...
        console.warn(`⚠️ Failed to get details for movieId ${movieId}`);
      }
    }
    ...
    if (!genreIds.length) {
      try {
        ...
        console.error("❌ Failed to fetch genre list:", err.message);
        return res.status(500).json({ message: "Could not resolve genres" });
      }
    }

    if (!genreIds.length) {
      console.warn("❌ No valid genre IDs. Returning empty.");
      return res.json([]);
    }
    ...
    try {
      ...
      console.warn("⚠️ TMDB discover call failed:", err.message);
    }

    // Retry without actor if no results
    if (!results.length && actorId) {
      console.log("🔁 Retrying discover without actor...");
